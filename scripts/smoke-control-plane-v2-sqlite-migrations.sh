...
assert fresh_init["database"]["current_version"] == 5, fresh_init
assert fresh_show["schema_version"]["tracked"] is True, fresh_show
assert fresh_show["schema_version"]["current_version"] == 5, fresh_show
...
assert legacy_migrate_1["migration"]["version_after"] == 5, legacy_migrate_1
assert [migration["version"] for migration in legacy_migrate_1["migration"]["executed_migrations"]] == [2, 3, 4, 5], legacy_migrate_1
...
assert legacy_after["schema_version"]["current_version"] == 5, legacy_after
