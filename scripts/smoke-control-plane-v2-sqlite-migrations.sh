...
assert fresh_init["database"]["current_version"] == 7, fresh_init
assert fresh_show["schema_version"]["tracked"] is True, fresh_show
assert fresh_show["schema_version"]["current_version"] == 7, fresh_show
...
assert legacy_migrate_1["migration"]["version_after"] == 7, legacy_migrate_1
assert [migration["version"] for migration in legacy_migrate_1["migration"]["executed_migrations"]] == [2, 3, 4, 5, 6, 7], legacy_migrate_1
...
assert legacy_after["schema_version"]["current_version"] == 7, legacy_after
